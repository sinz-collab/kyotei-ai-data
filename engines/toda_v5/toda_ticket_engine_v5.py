...
def combo_prob(combo, win, second_by_head, third_by_head):
    ...
    return round(num(win[str(a)])*num(second_by_head[str(a)][str(b)])*num(third_by_head[str(a)][str(c)])/10000,1)
...
def build_head_conditionals(head, scores, scenario):
    ...
    for lane in LANES:
        k=str(lane)
        if lane==head:
            second[k]=.03; third[k]=.03; continue
...
def build_tickets(win, second_by_head, third_by_head, scenarios, sab):
    heads=sorted(LANES,key=lambda x:win[str(x)],reverse=True)
    head_limit={"S":1,"A":2,"B":3}.get(sab,3)
    out=[]
    smap={s["head"]:s for s in scenarios}
    for h in heads[:head_limit]:
        s=smap.get(h,{})
        links=s.get("links") or [x for x in LANES if x!=h]
        ...
        for b in secs:
            ...
            for c in thirds:
                combo=f"{h}-{b}-{c}"
                out.append({"combo":combo,"role":"本線" if len(out)<3 else "展開保険",
                            "prob":combo_prob(combo,win,second_by_head,third_by_head),"odds":"-"})
    seen=[]; final=[]
    for x in out:
        if x["combo"] not in seen:
            seen.append(x["combo"]); final.append(x)
    return final[:6 if sab=="S" else 9 if sab=="A" else 10]

def build_upset_tickets(win, second_by_head, third_by_head, scenarios):
    heads=sorted([x for x in LANES if x!=1],key=lambda x:win[str(x)],reverse=True)[:2]
    out=[]
    for h in heads:
        ...
        for b in secs:
            ...
            for c in thirds:
                combo=f"{h}-{b}-{c}"
                out.append({"combo":combo,"role":"荒れ対応","prob":combo_prob(combo,win,second_by_head,third_by_head),"odds":"-"})
    return out[:8]
