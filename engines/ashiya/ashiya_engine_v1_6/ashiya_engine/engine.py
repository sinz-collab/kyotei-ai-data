...
from .scenario import detect,apply
...
class AshiyaEngine:
    ...
    def predict(self,race,stage='pre'):
        ...
        for i,r in enumerate(racers,1):
            ...
            r.setdefault('actual_course',r.get('entry_course') or r['lane'])
        ...
        audit={'model':model_audit,'database':db_audit,'day_weighting':day_audit,'practical_support':support_audit,'entry_changed':entry_changed,'actual_entry_complete':all(r.get('actual_course') for r in racers),'odds_used':False,'full_reflection':True}
        sab=grade(final,structure,audit); tickets=generate(final,structure)
        audit.update({'ticket_count':len(tickets['all']),'duplicate_count':len(tickets['all'])-len({x['combination'] for x in tickets['all']}),'normalized':all(abs(sum(final[k])-1)<1e-8 for k in final)})
        probs=[]
        ...
        return {'engine':{'name':'ashiya_prediction_engine','version':'1.6.0','master_version':'v4.2','stage':stage,'generated_at':datetime.now(JST).isoformat()},'race':{'date':race.get('date') or race.get('race_date'),'race_no':race.get('race_no') or race.get('race')},'probabilities':probs,'attack_structure':{k:v for k,v in structure.items() if k!='scenarios'},'scenarios':structure['scenarios'],'sab':sab,'tickets':tickets,'audit':audit}
