...
def prediction_complete(prediction: dict) -> bool:
    ...
    for key in ("win", "second", "third"):
        ...
        if not isinstance(values, dict) or sorted(map(int, values.keys())) != list(range(1, 7)):
            return False
        if abs(sum(float(v) for v in values.values()) - 100.0) > 0.2:
            return False
    return True
...
def apply_toda_v5(payload: dict, target_date: str) -> dict:
    ...
    if failures:
        raise RuntimeError(
            "toda_v5_generation_failed: "
            + json.dumps(failures, ensure_ascii=False)
        )
    if sorted(map(int, predictions.keys())) != list(range(1, 13)):
        raise RuntimeError("toda_v5_predictions_must_be_12")
...
def main() -> int:
    ...
    parser.add_argument(
        "--require-open",
        action="store_true",
        help="Fail when Toda JSON does not exist instead of skipping.",
    )
    args = parser.parse_args()

    date_dir = args.date.replace("-", "")
    data_root = Path(args.data_root)
    dated_path = data_root / "venues" / "toda" / f"{date_dir}.json"
    latest_path = data_root / "venues" / "toda" / "latest.json"

    if not dated_path.exists():
        message = f"Toda data is not open: {dated_path}"
    ...
    payload = apply_toda_v5(payload, args.date)
    ...
    summary = {
        ...
        "sourceSummary": {
            race_no: prediction.get("sourceSummary", {})
            for race_no, prediction in payload["preds"].items()
        },
    }
