...
ENGINE_DIR = REPO_ROOT / "engines" / "biwako_v1_1"
CONFIG_PATH = ENGINE_DIR / "biwako_correction_v1_1.json"
DEFAULT_DB_PATH = REPO_ROOT / "data" / "venues" / "biwako" / "db" / "biwako_ai_master.sqlite"
ENGINE_ID = "biwako_engine_v1.1"
ENGINE_VERSION = "1.1"

if str(ENGINE_DIR) not in sys.path:
    sys.path.insert(0, str(ENGINE_DIR))

from biwako_prediction_engine_v1_1 import BiwakoPredictionEngineV11  # noqa: E402
...
def format_prediction(result: dict, phase: str) -> dict:
    ...
    stage = {
        ...
        "statusText": (
            "直前・展示・オリジナル展示を反映してびわこv1.1で再予想済み"
            if is_final
            else "前データをびわこv1.1へ反映。直前データ取得後に本予想へ更新"
        ),
...
def apply_predictions(
    payload: dict,
    target_date: str,
    engine: BiwakoPredictionEngineV11,
    resolver: PlayerIdResolver,
    stage: str,
    live_base: Path,
...
def main() -> int:
    ...
    engine = BiwakoPredictionEngineV11(db_path, CONFIG_PATH)
    try:
