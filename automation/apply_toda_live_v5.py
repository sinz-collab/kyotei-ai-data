...
def sync_race_prediction(payload: dict, race_no: int, prediction: dict, engine_id: str) -> None:
    ...
    envelope.update({
        ...
        "probabilities": {key: deepcopy(prediction[key]) for key in ("win", "second", "third")},
        "sab": prediction.get("sab"),
        "tickets": {key: deepcopy(prediction.get(key)) for key in ("ai", "aiUpset", "balance", "tickets") if isinstance(prediction.get(key), list)},
...
def apply_toda_live_review(payload: dict, target_date: str, race_no: int, live_root: Path) -> dict:
    if payload.get("venueId") != "toda" or payload.get("date") != target_date:
        raise RuntimeError("toda_payload_identity_invalid")
    prediction = (payload.get("preds") or {}).get(str(race_no))
    ...
    documents = {"direct": direct, "exhibition": exhibition}
...
def apply_file(target_date: str, race_no: int, data_root: Path, live_root: Path) -> dict:
    ...
    return {"date": target_date, "race": race_no, "engine": payload["engine"], "reviewed": prediction["probabilityFlow"]["reviewed"], "realtimeApplied": prediction["probabilityFlow"]["realtimeApplied"], "originalExhibitionApplied": prediction["liveReviewMeta"]["originalExhibitionApplied"], "oddsUsedForProbability": prediction["liveReviewMeta"]["oddsUsedForProbability"], "datedPath": str(dated_path)}
...
def main() -> int:
    ...
    print(json.dumps(apply_file(args.date, args.race, args.data_root, args.live_root), ensure_ascii=False, indent=2))
    return 0
